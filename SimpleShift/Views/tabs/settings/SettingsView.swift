...
//                Section {
//                    Button("Send watch data") {
//                        WatchConnectivityManager.shared.sendSimulatorTest()
//                    }
//                }
//                #endif
                ...
                footer: {
                    ZStack {
                        HStack {
                            Text("SimpleShift v2.0alpha1\n© 2023 Ollie Munday")
                                .padding(.vertical, 10)
                            Spacer()
                        }
                        Rectangle()
                            .hidden()
                            .overlay { SpinningGradientLogo(size: 120).offset(y: 240) }
                    }
                }
            ...
            .navigationDestination(for: Navigation.self) { value in
                ...
                case .applewatch : Text("Apple Watch")
                }
            }
        }
    ...
    private var weekdaySelection: some View {
        List {
            ForEach(weekdayUtil.weekdays_long.indices, id: \.self) { index in
                let weekday = weekdayUtil.weekdays_long[index]
                Button {
        ...
        ScrollView {
            ...
            LazyVGrid(columns: [GridItem(spacing: 16), GridItem(spacing: 16)], spacing: 16) {
                ForEach(0..<5) { index in
                    Button {
                        withAnimation {
                            settingsManager.todayIndicatorType = index
                        }
                    } label: {
                        ZStack {
                            ...
                            VStack(spacing: 16) {
                                DateView(id: index,
                                         calendarDisplay: CalendarDisplay(id: index,
                                                                          date: CalendarDate(id: index, date: Date.now),
                                                                          shift: nil,
                                                                          day: "12",
                                                                          isGreyed: false,
                                                                          showOff: settingsManager.calendarShowOff,
                                                                          indicatorType: index),
